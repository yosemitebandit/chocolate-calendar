...
type decompressV2TestInstance struct {
	input        string
	outputLength int
}

var decompressV2TestInstances = []decompressV2TestInstance{
	{"(3x3)XYZ", 9},
	{"X(8x2)(3x3)ABCY", 20},
...
func TestDecompressV2(t *testing.T) {
	for _, test := range decompressV2TestInstances {
		result := decompressV2(test.input)
		if len(result) != test.outputLength {
			t.Error("Fail, input:", test.input, "Got:", result)
		}
	}
}
